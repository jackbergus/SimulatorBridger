...
public class Switch extends SimEntity {
	...
	protected void processpacket_down(SimEvent ev) {
		...
		MainEventManager.cancelAll(getId(), new PredicateType(CloudSimTags.Network_Event_send));
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == NetworkConstants.ROOT_LEVEL) {
			...
			if (aggSwtichid < 0) {
				logger.debug(" No destination for this packet");
			} else {
	...
	protected void processpacket(SimEvent ev) {
		...
		MainEventManager.cancelAll(getId(), new PredicateType(CloudSimTags.Network_Event_UP));

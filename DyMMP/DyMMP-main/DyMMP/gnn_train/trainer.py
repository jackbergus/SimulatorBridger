...
class Trainer():
    def __init__(self, model, train_dataloader, val_dataloader, test_dataloader, config, loss_function, device):
        ...
        self.test_results = {"predictions" : [], "ground_truth" : [],"all_predictions" : [], "all_ground_truth" : []}
        self.epoch_times = []
    ...
    def train(self, optimizer, scheduler=None):
        ...
        for epoch in range(self.epochs):
            ...
            for i, data in enumerate((pbar := tqdm(self.train_dataloader))):
                ...
                if (self.val_steps == 0) or (i % self.val_steps == self.val_steps - 1):
    ...
    def evaluate(self, f, dataset):
        ...
        with torch.no_grad():
            ...
            for data in tqdm(dataset):
                ...
                # Save ground truth and predictions for later use
                gt_list, pred_list = self.batch_to_list(data, preds)
                self.test_results["all_ground_truth"].extend(gt_list)
                self.test_results["all_predictions"].extend(pred_list)
        ...
        f.write(f"Test Loss (L1/MAE): {mean_test_loss:.4f}"+os.linesep)
        f.write(f"Test Loss (L2/RMSE): {mean_test_loss_rmse:.4f}"+os.linesep)

    def batch_to_list(self, data, preds):
        pass

    def dump_data_all(self, f, dataset):
        preds = pandas.DataFrame(self.test_results["all_predictions"])
        truth = pandas.DataFrame(self.test_results["all_ground_truth"])
        station_names = dataset.dataset.column_names
        preds.columns = station_names
        truth.columns = station_names
        preds.index = dataset.dataset.timestamps[:len(preds.index)]
        truth.index = dataset.dataset.timestamps[:len(preds.index)]
        preds_file = os.path.join(self.prediction_plot_dir, "all_predictions.csv")
        preds.to_csv(preds_file)
        orig_file = os.path.join(self.prediction_plot_dir, "all_original.csv")
        truth.to_csv(orig_file)
        from dtwParallel import dtw_functions
        f.write(f"DTW (parallel): {dtw_functions.dtw(preds.values, truth.values, MTS=True, n_threads=8)} [https://www.sciencedirect.com/science/article/pii/S2352711023000602]"+os.linesep)
        from sktime.distances import wddtw_distance
        f.write(
            f"WDDTW: {wddtw_distance(preds.values, truth.values)} [https://www.sciencedirect.com/science/article/pii/S003132031000484X?via%3Dihub]" + os.linesep)
    ...
    def save_prediction_plot(self, from_index, length):
        ...
        fig, ax = plt.subplots(figsize=(12,6))
        ts_shape = timestamps.shape[0]
        tr_shape = len(truth[: , station_indices[0]])
        upper = min(ts_shape, tr_shape)
        for i, station_id in zip(station_indices, station_ids): 
            ax.plot(timestamps[:upper], truth[:upper, i], label="Ground truth", c="blue", alpha=0.7)
            ax.plot(timestamps[:upper], preds[:upper, i], label="Predicted", c="red", alpha=0.7)
...
class GNNTrainer(Trainer):
    ...
    def print_model_size(self, f):
        f.write(f"Model size: {get_model_size(self.model)/2**20:.2f} MB"+os.linesep)
        f.write(f"Parameters: {count_parameters(self.model)}"+os.linesep)

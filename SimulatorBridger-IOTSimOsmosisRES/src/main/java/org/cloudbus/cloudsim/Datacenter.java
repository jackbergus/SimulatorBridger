...
public class Datacenter extends SimEntity {
	...
	/** The Mel list. */
//	private List<? extends MicroELement> melList;
	 ...
	 *                   CloudResource must contain one or more Machines. A Machine
	 *                   must contain one or more PEs.
	 *                   </ul>
	 * @pre name != null
	 * @pre resource != null
	 * @post $none
...
//			break;
		...
		case CloudSimTags.CLOUDLET_SUBMIT_IoTSim:
			processCloudletSubmitIoTSim(ev, false);
			break;

		// New Cloudlet arrives, but the sender asks for an ack
		case CloudSimTags.CLOUDLET_SUBMIT_IoTSim_ACK:
			processCloudletSubmitIoTSim(ev, true);
			break;
...
//			break;
	...
	if (ack) {
			...
			send(vm.getUserId(), MainEventManager.getMinTimeBetweenEvents(), CloudSimTags.VM_CREATE_ACK, data);
		...
		if (result) {
			...
			vm.updateVmProcessing(MainEventManager.clock(),
...
//		}
	...
	protected void processCloudletResume(int cloudletId, int userId, int vmId, boolean ack) {
		...
		if (eventTime > 0.0) { // if this cloudlet is in the exec queue
			status = true;
			if (eventTime > MainEventManager.clock()) {
	...
	protected void updateCloudletProcessing(double deltaTime) {
		...
		if (MainEventManager.clock() > getLastProcessTime() + MainEventManager.getMinTimeBetweenEvents()) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				double time = host.updateVmsProcessing(MainEventManager.clock());
			...
			if (smallerTime < MainEventManager.clock() + MainEventManager.getMinTimeBetweenEvents() + 0.01) {
				smallerTime = MainEventManager.clock() + MainEventManager.getMinTimeBetweenEvents() + 0.01;
			}
			if (smallerTime != Double.MAX_VALUE) {
				schedule(getId(), (smallerTime - MainEventManager.clock()), CloudSimTags.VM_DATACENTER_EVENT);
			}
			setLastProcessTime(MainEventManager.clock());
		}
	}
	 ...
	 */
	protected boolean contains(FileSIM file) {
		if (file == null) {
			return false;
		}
	...
	public void startEntity() {
		...
		int gisID = MainEventManager.getEntityId(regionalCisName);
		if (gisID == -1) {
			gisID = MainEventManager.getCloudInfoServiceEntityId();
		}
	 ...
	 */
	/*
	 * protected DatacenterCharacteristics getCharacteristics() { return
	 * characteristics; }
	 */

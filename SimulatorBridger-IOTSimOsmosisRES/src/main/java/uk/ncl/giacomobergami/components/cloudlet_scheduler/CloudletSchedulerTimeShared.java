...
//				OsmoticWrapper.melList.put(mel, getMipsShare);
	...
	public double cloudletResume(int cloudletId) {
		...
		if (found) {
			...
			double estimatedFinishTime = MainEventManager.clock()
	...
	//	java.text.DecimalFormat   df=new  java.text.DecimalFormat("#.######");   
		...
		/*double length = (double) (cloudlet.getCloudletLength() + extraSize);
		  ...
		  bd   =   bd.setScale(11,BigDecimal.ROUND_CEILING); 
	...
	public double getCurrentRequestedUtilizationOfRam() {
		...
		for (ResCloudlet cloudlet : cloudletExecList) {
			ram += cloudlet.getCloudlet().getUtilizationOfRam(MainEventManager.clock());
		}
		return ram;
	}
	...
	public double getCurrentRequestedUtilizationOfBw() {
		double bw = 0;
		for (ResCloudlet cloudlet : cloudletExecList) {
			bw += cloudlet.getCloudlet().getUtilizationOfBw(MainEventManager.clock());
		}
		return bw;
	}

...
def create_edge_index_and_features(stations_included_file, stations_data_file, graph_file=None, compute_edge_features=True):
    ...
    """
    stations_included = [str(x) for x in pd.read_csv(stations_included_file, index_col=0).iloc[:, 0]]
    stations_data_df = pd.read_csv(stations_data_file) 
    # positions = stations_data_df.loc[stations_data_df["idx"].isin([int(x) for x in stations_included]), ["lat", "lon"]].to_numpy()
    ...
    graph_df = pd.read_csv(graph_file, index_col=0)[stations_included].loc[[int(x) for x in stations_included]]
        ...
        for j in range(i+1, num_nodes):
            if graph_df.iloc[i,j]:
                ...
                if compute_edge_features:
                    edge_feature = np.exp(-graph_df[stations_included[i]][int(stations_included[j])])
                else:
                    edge_feature = 1.0

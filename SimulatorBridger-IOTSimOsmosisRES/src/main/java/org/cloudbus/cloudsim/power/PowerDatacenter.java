...
public class PowerDatacenter extends Datacenter {
	...
	protected double updateCloudetProcessingWithoutSchedulingFutureEvents() {
		if (MainEventManager.clock() > getLastProcessTime()) {
	...
	protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
		...
		logger.debug("\n\n--------------------------------------------------------------\n\n");
		logger.debug("New resource usage for the time frame starting at %.2f:", currentTime);
...
logger.debug(
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		SimEvent event = MainEventManager.findFirstDeferred(getId(), new PredicateType(CloudSimTags.VM_MIGRATE));
		if (event == null || event.eventTime() > MainEventManager.clock()) {

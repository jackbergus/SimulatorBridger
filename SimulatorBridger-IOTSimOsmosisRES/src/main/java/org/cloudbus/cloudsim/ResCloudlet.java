...
public class ResCloudlet implements Serializable {
	...
	private double cloudletFinishedSoFar;
	...
	private void init() {
		...
		arrivalTime = MainEventManager.clock();
	...
	public boolean setCloudletStatus(int status) {
		...
		try {
			double clock = MainEventManager.clock();   // gets the current clock
	...
	public double getRemainingCloudletLength() {
		double length = cloudlet.getCloudletTotalLength() * Consts.MILLION - cloudletFinishedSoFar;
		...
		double temp = (double) Math.floor(length / Consts.MILLION);		
	...
	public void finalizeCloudlet() {
		...
		double wallClockTime = MainEventManager.clock() - arrivalTime;
	...
	public void updateCloudletFinishedSoFar(double miLength) {

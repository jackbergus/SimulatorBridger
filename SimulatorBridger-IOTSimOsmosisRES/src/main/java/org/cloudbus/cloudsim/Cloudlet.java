...
public class Cloudlet implements Serializable {
	...
	public Cloudlet(
			...
			final double cloudletLength,
			final int pesNumber,
			final double cloudletFileSize,
			final double cloudletOutputSize,
	 ...
	 * @post $none
	 */
	public boolean setCloudletLength(final double cloudletLength) {
		if (cloudletLength <= 0) {
			return false;
		}
	 ...
	 * @post $result >= 0.0
	 */
	public double getCloudletFinishedSoFar() {
		if (index == -1) {
			return cloudletLength;
		}

		final double finish = resList.get(index).finishedSoFar;
	...
	public boolean isFinished() {
		...
		final double finish = resList.get(index).finishedSoFar;
		final double result = cloudletLength - finish;
		if (result <= 0.0) {
			completed = true;
		}
		return completed;
	 ...
	 * @post $none
	 */
	public void setCloudletFinishedSoFar(final double length) {
	...
	public void setUserId(final int id) {
		userId = id;
		if (record) {
			write("Assigns the Cloudlet to " + MainEventManager.getEntityName(id) + " (ID #" + id + ")");
		}
	}
	 ...
	 * @post $result >= 1
	 */
	public double getCloudletFileSize() {
	 ...
	 * @post $result >= 1
	 */
	public double getCloudletOutputSize() {
	...
	public void setResourceParameter(final int resourceID, final double cost) {
		...
		res.costPerSec = cost;
		res.resourceName = MainEventManager.getEntityName(resourceID);
		...
		if (index == -1 && record) {
			...
			/*System.out.println("Allocates this Cloudlet" + this.cloudletId + " to " + res.resourceName + " (ID #" + resourceID
					+ ") with cost = $" + cost + "/sec");*/
	...
	public void setSubmissionTime(final double clockTime) {
		...
		if (record) {
			...
			//System.out.println("Sets the submission time to " + num.format(clockTime));
		}
	}
	...
	public void setExecStartTime(final double clockTime) {
		...
		if (record) {
			...
			//System.out.println("Sets the execution start time to " + num.format(clockTime));
		}
	}
	...
	public void setExecParam(final double wallTime, final double actualTime) {
		...
		if (record) {
			...
			/*System.out.println("Sets the wall clock time to " + num.format(wallTime) + " and the actual CPU time to "
					+ num.format(actualTime));*/
		}
	}
	...
	public void setCloudletStatus(final int newStatus) throws Exception {
		...
		if (newStatus == Cloudlet.SUCCESS) {
			finishTime = MainEventManager.clock();
		}
		...
		/*	System.out.println("Sets Cloudlet status from " + getCloudletStatusString() + " to "
					+ Cloudlet.getStatusString(newStatus));*/
	...
	protected void write(final String str) {
		...
		if (num == null || history == null) { // Creates the history or
			...
			history.append("------------------------------------------");
			history.append(System.getProperty("line.separator"));
			history.append(num.format(MainEventManager.clock()));
	...
	/**
	 * Sets the ID of the Host that will run this Cloudlet.
	 * 
...
/*	public double getProcessingCost() {
	...
	public double getTotalProcessingCost() {
		// cloudlet cost: execution cost...
		// double cost = getProcessingCost();
		double cost = this.getCPUProcessingCost();
		// ...plus input data transfer cost...
		cost += accumulatedBwCost;
		// ...plus output cost
		cost += costPerBw * getCloudletOutputSize();
	...
	public double getTotalBWCost() {
		// total bw cost = input transfer cost + output transfer cost
		
		double totalBWCost = this.accumulatedBwCost;
	
		totalBWCost += costPerBw * getCloudletOutputSize();
		return totalBWCost;
	...
	public double getCPUProcessingCost() {
		// cloudlet cost: execution cost...
		// double cost = getProcessingCost();
	...
	public int getVmTypeforMRCloudSim() {
